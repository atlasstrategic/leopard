...
**Wait for the fuel berth first.** The attempt opens with a radio call: a monohull is refuelling at the berth. Motor to the blue **holding area** south-east of the quay (instrument caption **BRG HOLD** gives bearing and distance) and keep the boat's centre inside it. After **5 seconds** in position the monohull departs; leaving the area during the countdown resets it. Stay out of the fuel berth and its approach lane until the radio calls it clear: each early entry costs **+10 s**. See [Harbour traffic](#harbour-traffic-milestone-c-in-progress) for the full rules, including mission failure on unprotected contact with the monohull.
...
The fuel mission ([issue #1](https://github.com/atlasstrategic/leopard/issues/1)) now starts with **holding and clearance**, announced step by step on the **Harbour radio** panel at the top of the screen (also logged as `radio` events):
...
3. **Departure:** the monohull casts off. It is never delayed at random.
4. **Clearance:** once no part of the monohull is inside the fuel berth and its approach lane (x −4…7 m, y −10…37 m), the radio calls the berth clear and the normal **approach** objective begins. In practice this is about 50 s after departure.

Entering that fuel zone (boat centre) before clearance costs **+10 s** per entry and is logged as `mission.early_entry`. Line attachment is refused until the berth is clear.

**Contact with the monohull:** where a deployed fender covers the point of contact (the existing local fender coverage), it is an ordinary contact penalty (+5 s above 0.08 m/s). Contact on any uncovered part of the hull is **mission failed**: the simulation stops, crew commands are refused and only **Retry** continues. This is a game rule for another vessel, not a damage model.
...
Published at **https://atlasstrategic.github.io/leopard/** from the public [`atlasstrategic/leopard`](https://github.com/atlasstrategic/leopard) repository. GitHub Pages uses the **GitHub Actions** deployment source; pushing to `main` triggers a new deployment after checks pass. The Action's `deploy` job reports the Pages URL. If you fork or move the repository, configure Pages for Actions and check the project URL and `base` path again. The workflow uses only the token GitHub grants it, scoped to Pages deployment. Preview locally with `npm run build && npx vite preview` and open the reported `/leopard/` URL. A custom domain or a differently named repository requires updating the Vite `base` and Pages settings before deploying; this configuration intentionally targets the project URL `/leopard/`.
